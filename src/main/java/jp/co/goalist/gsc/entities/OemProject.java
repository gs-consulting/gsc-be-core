...
@NoArgsConstructor
@AllArgsConstructor
@Cache(region = "oemProjectCache", usage = CacheConcurrencyStrategy.READ_WRITE)
@Entity(name = "oem_projects")
public class OemProject extends BaseEntity {
    ...
    @ManyToMany
    @JoinTable(
            name = "oem_advertisement_linkings",
            joinColumns = @JoinColumn(name = "project_id"),
            inverseJoinColumns = @JoinColumn(name = "advertisement_id"))
    private List<OemAdvertisement> advertisements = new ArrayList<>();

    @ManyToMany
    @JoinTable(
            name = "oem_project_linkings",
            joinColumns = @JoinColumn(name = "project1_id"),
            inverseJoinColumns = @JoinColumn(name = "project2_id"))
    private List<OemProject> linkingProjects = new ArrayList<>();

    public void setLinkingProjects(List<OemProject> linkingProjects) {
