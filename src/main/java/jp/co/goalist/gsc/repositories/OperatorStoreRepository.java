...
public interface OperatorStoreRepository extends JpaRepository<OperatorStore, String> {
    ...
    @Query(value = """
            ...
            nativeQuery = true,
            countQuery = """
            SELECT count(*) FROM operator_stores WHERE id IN (
            SELECT s.id FROM operator_stores s LEFT JOIN prefectures p ON s.prefecture_id = p.id
            LEFT JOIN cities c ON s.city_id = c.id
            LEFT JOIN operator_branches b ON s.branch_id = b.id
            WHERE (CASE WHEN :clientAccountId IS NULL THEN 1=1 ELSE s.parent_id = :clientAccountId OR s.parent_id IS NULL END)
    ...
    @Query(value = """
        SELECT * FROM operator_stores s
        WHERE (COALESCE(:branchIds) IS NULL OR s.branch_id IN (:branchIds))
        AND (s.parent_id IS NULL OR (:clientAccountId IS NULL OR s.parent_id = :clientAccountId))
        ORDER BY s.created_at ASC
    """, nativeQuery = true)
    ...
    @Modifying
    @Query(value = "DELETE FROM operator_stores WHERE branch_id IN (:branchIds) AND parent_id = :parentId", nativeQuery = true)
    void deleteStoresByBranchIds(List<String> branchIds, String parentId);

    @Modifying
    @Query(value = "DELETE FROM operator_stores WHERE id IN (:storeIds) AND parent_id = :parentId", nativeQuery = true)
    void deleteStoresByIds(List<String> storeIds, String parentId);
}
